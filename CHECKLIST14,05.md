...
| Отображение приветствия | Приветствие должно меняться в зависимости от времени суток | Запустить приложение в разное время суток (утро/день/вечер/ночь) | | | Fails | ! нету окна приветствия соответственно Не меняет статус на вечер,  день, ночь
...
| Функция/Экран | Что должно работать | Проверка | Статус | Комментарии/Ошибки |
|---------------|---------------------|----------|--------|-------------------|
| Список дыхательных практик | Должен отображаться список доступных шаблонов | Открыть экран дыхательных практик |FAILS |Нету ни одной практики , если нажимаешь на дыхание, заходит только тогда, когда нажимаешь на "быстрая медитация"|
| Запуск практики | При выборе шаблона должна запускаться практика | Выбрать шаблон из списка | N/A | Нету дыхательных практик |
| Анимация дыхания | Анимация должна соответствовать текущей фазе дыхания (вдох/выдох/задержка) | Запустить практику и наблюдать за анимацией |N/A| Нет анимации |
| Голосовые подсказки | Должны воспроизводиться голосовые подсказки для каждой фазы | Запустить практику с включенными голосовыми подсказками | N/A | Нет  |
| Управление практикой | Кнопки паузы/возобновления/остановки должны работать | Проверить работу каждой кнопки во время практики | N/A | Нет |
| Адаптивное дыхание | Компонент адаптивного дыхания должен предлагать шаблоны | Проверить отображение и функциональность | N/A | Ничего не предлагает |
| Аналитика дыхательных сессий | Должна отображаться статистика сессий | Проверить отображение после завершения практики | N/A| Аналитика дыхательных сессий "ваш прогресс" должна быть вынесена в статистику |
| Голосовое управление | Команды "Начать дыхание" и "Стоп" должны работать | Произнести команды и проверить реакцию | N/A| Нету |
| Музыкальное сопровождение | Должна быть возможность выбора музыки с разным ритмом | Проверить выбор музыки и её воспроизведение | N/A | Нету |
| Переход к созданию шаблона | Кнопка создания шаблона должна перенаправлять на PatternBuilder | Нажать на кнопку создания шаблона | N/A | Не реализовано |

## 3. Создание шаблона дыхания (PatternBuilder)

| Функция/Экран | Что должно работать | Проверка | Статус | Комментарии/Ошибки |
|---------------|---------------------|----------|--------|-------------------|
| Интерфейс создания | Должны отображаться элементы для настройки параметров | Открыть экран создания шаблона | N/A | Нету |
| Настройка параметров | Должна быть возможность настройки длительности фаз | Изменить параметры и проверить обновление предпросмотра | N/A | Нету |
